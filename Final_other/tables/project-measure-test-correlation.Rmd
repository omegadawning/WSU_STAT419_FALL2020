...
file.correlation.2 = paste0(path.tables,"tree-correlation-table-2.tex");

myData = as.matrix(final_male[,c(1,2,3,4,5,10)]);  # numeric values only, only what will appear in table
myData.2 = as.matrix(final_female[,c(1,2,3,4,5,10)]);
...
buildLatexCorrelationTable(myData, 
  rotateTable = TRUE,
  width.table = 0.75, # best for given data ... 0.95 when rotateTable = FALSE
                      # 0.60 when rotateTable = TRUE
  myFile = file.correlation,
  myNames = c("Height (cm)", "L Hand Len. (cm)", "R Hand Len. (cm)", 
              "L Hand Width (cm)", "R Hand Width (cm)", 
              "Wingspan (cm)"),
  myCaption = "Descriptive Statistics and Correlation Analysis for Males");

buildLatexCorrelationTable(myData.2, 
  rotateTable = TRUE,
  width.table = 0.75, # best for given data ... 0.95 when rotateTable = FALSE
                      # 0.60 when rotateTable = TRUE
  myFile = file.correlation.2,
  myNames = c("Height (cm)", "L Hand Len. (cm)", "R Hand Len. (cm)", 
              "L Hand Width (cm)", "R Hand Width (cm)", 
              "Wingspan (cm)"),
